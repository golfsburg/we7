...
<div class="fcard" style="border-color:rgba(167,139,250,.25)">
  <div class="fcard-t">📂 PVGIS CSV Dateien laden</div>
  <p style="font-size:13px;color:var(--tx2);margin-bottom:12px">
    Unter <a href="https://re.jrc.ec.europa.eu/pvg_tools/en/" target="_blank" style="color:var(--ac)">PVGIS Tool</a>
    → Tages-Strahlungsprofil → Koordinaten und Neigung/Azimuth eingeben → CSV exportieren und hier laden.<br>
    Mehrere Dateien gleichzeitig wählbar (verschiedene Neigungen / Ausrichtungen).
  </p>
  <div style="margin-bottom:10px">
    <input type="file" id="sim-file" accept=".csv,.txt" multiple
      style="font-family:var(--font-m);font-size:12px;color:var(--tx2);background:var(--bg3);
             border:1px solid var(--b2);border-radius:8px;padding:8px 10px;width:100%;cursor:pointer"
      onchange="SIM.loadFiles(this)">
  </div>
  <div id="sim-file-list" style="font-size:12px;color:var(--tx2);margin-bottom:10px"></div>
  <div class="brow">
    <button class="btn-p" onclick="SIM.render()">Simulation anzeigen</button>
    <button class="btn-s" onclick="SIM.clearAll()">Alle entfernen</button>
  </div>
</div>

<!-- Szenarien-Liste -->
<div id="sim-scenarios" style="margin-bottom:16px"></div>

<!-- Metriken -->
<div id="sim-metrics" style="display:none">
  <div class="metrics" id="sim-mc"></div>

  <!-- Stunden-Profil Chart -->
  <div class="cc full" style="margin-bottom:14px">
    <div class="cc-title">Stündliches Strahlungsprofil G(i)</div>
    <div class="cc-sub" id="sim-chart-sub">W/m² pro Stunde — alle Szenarien überlagert</div>
  ...
  <div class="cgrid">
    <div class="cc full">
      ...
      <div class="cc-sub">kWh über den Tag — basierend auf G(i), Anlagenfläche und Wirkungsgrad</div>
    ...
    <div class="cc">
      ...
      <div class="cc-sub">Anteil Gb(i) / Gd(i) im besten Szenario</div>
      <div class="cw" style="height:220px"><canvas id="sim-c-donut"></canvas></div>
    </div>
  </div>

  <!-- Einstellungen für Schätzung -->
  <div class="fcard">
    ...
    <div class="fgrid" style="grid-template-columns:1fr 1fr 1fr 1fr">
      <div class="field"><label>Nennleistung (Wp)</label><input type="number" id="sim-wp" value="800" step="10" onchange="SIM.render()"></div>
      <div class="field"><label>Wirkungsgrad (%)</label><input type="number" id="sim-eff" value="18" step="1" min="1" max="30" onchange="SIM.render()"></div>
      <div class="field"><label>Performance Ratio</label><input type="number" id="sim-pr" value="0.82" step="0.01" min="0.1" max="1.0" onchange="SIM.render()"></div>
      <div class="field"><label>Fläche (m²)</label><input type="number" id="sim-area" value="4" step="0.1" min="0.1" onchange="SIM.render()"></div>
    </div>
    <div class="note">💡 Ertrag ≈ G(i) × Fläche × PR / 1000 (kWh). Oder: G(i)/1000 × Wp × PR (beide Methoden verfügbar).</div>
  </div>
</div>
`;

// ── STATE ───────────────────────────────────────────────
let scenarios = []; // [{name, label, month, slope, azimuth, data:[{hour,gi,gbi,gdi}]}]
...
function loadFiles(input) {
  ...
  if (listEl) listEl.innerHTML = `Lade ${files.length} Datei(en)...`;
  let loaded = 0;
  ...
  files.forEach(file => {
    ...
    reader.onload = e => {
      ...
      const parsed = parseCSV(raw, file.name);
      if (parsed) {
        scenarios = scenarios.filter(s => s.name !== parsed.name);
        scenarios.push(parsed);
        console.log(`SIM loaded: ${file.name} → ${parsed.data.length} rows, label: ${parsed.label}`);
      } else {
        console.warn(`SIM: could not parse ${file.name}`);
        console.warn(`First 200 chars: ${raw.substring(0,200)}`);
      }
      loaded++;
      if (loaded === files.length) {
        if (listEl) listEl.innerHTML = scenarios.length > 0
          ? `<span style="color:var(--gr)">✓ ${scenarios.length} Szenario(s) geladen</span>`
          : `<span style="color:var(--rd)">⚠ Keine gültigen PVGIS-Dateien erkannt — Details in der Browser-Konsole (F12)</span>`;
        render();
      }
    };
...
function parseCSV(raw, filename) {
  // Strip BOM, normalize all line endings and whitespace
  const cleaned = raw
    .replace(/^\uFEFF/, '')
    .replace(/\r\n/g, '\n')
    .replace(/\r/g, '\n');
  ...
  for (const line of lines) {
    ...
    // Data row: first column must be HH:MM (time)
    if (/^\d{1,2}:\d{2}$/.test(first) && parts.length >= 4) {
      ...
      if (!isNaN(hour) && !isNaN(gi)) {
        data.push({ hour, gi, gbi, gdi });
      }
    }
  }

  if (!data.length) {
    console.warn(`[SIM] parseCSV failed for "${filename}". Parsed ${lines.length} lines, no data rows found.`);
    return null;
  }
  ...
  console.log(`[SIM] parsed "${filename}": ${data.length} rows, label="${label}"`);
  return { name: filename, label, ...meta, data };
}
...
function render() {
  ...
  const wp  = parseFloat(document.getElementById('sim-wp')?.value)  || 800;
  ...
  // Calculate daily yield per scenario: sum(G(i)) / 1000 * area * PR
  const yields = scenarios.map(s => {
    const sumGi = s.data.reduce((a,d) => a+d.gi, 0);
    return +(sumGi / 1000 * area * pr).toFixed(3); // kWh/day
  });
  ...
  // Scenario cards
  if (scenariosEl) {
    scenariosEl.innerHTML = `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px;margin-bottom:16px">` +
      scenarios.map((s,i) => `
        <div style="background:var(--bg2);border:1px solid ${i===bestIdx?COLORS[i%COLORS.length]+'66':'var(--b1)'};border-radius:var(--rl);padding:14px;position:relative">
          <div style="position:absolute;top:10px;right:10px;cursor:pointer;font-size:12px;color:var(--tx3)" onclick="SIM.removeScenario(${i})">✕</div>
          <div style="width:10px;height:10px;border-radius:50%;background:${COLORS[i%COLORS.length]};margin-bottom:8px;box-shadow:0 0 6px ${COLORS[i%COLORS.length]}"></div>
          <div style="font-size:12px;font-weight:500;color:var(--tx);margin-bottom:4px">${s.label}</div>
          <div style="font-size:11px;color:var(--tx3)">${s.month} · ${s.lat}°N ${s.lon}°E</div>
          <div style="font-size:18px;font-weight:500;color:${COLORS[i%COLORS.length]};margin-top:8px">${yields[i]} <span style="font-size:11px;color:var(--tx2)">kWh/Tag</span></div>
          ${i===bestIdx?'<div style="font-size:10px;color:var(--gr);margin-top:4px">✓ Bestes Szenario</div>':''}
        </div>`).join('') + `</div>`;
  }
  ...
  if (mcEl) {
    ...
    const diff = +(maxYield - minYield).toFixed(3);
    const annualBest = +(maxYield * 365).toFixed(0);
    mcEl.innerHTML = `
      <div class="mc hi"><div class="mc-l">Bester Tagesertrag</div><div><span class="mc-v">${maxYield}</span><span class="mc-u">kWh</span></div><div class="mc-d">${best.label}</div></div>
      <div class="mc"><div class="mc-l">Schlechtester</div><div><span class="mc-v">${minYield}</span><span class="mc-u">kWh</span></div></div>
      <div class="mc gr"><div class="mc-l">Differenz</div><div><span class="mc-v">${diff}</span><span class="mc-u">kWh</span></div><div class="mc-d">${(diff/maxYield*100).toFixed(1)}% Unterschied</div></div>
      <div class="mc"><div class="mc-l">Hochrechnung/Jahr</div><div><span class="mc-v">${annualBest}</span><span class="mc-u">kWh</span></div><div class="mc-d">bestes Szenario</div></div>`;
  }

  const subtitle = document.getElementById('sim-chart-sub');
  if (subtitle) subtitle.textContent = `W/m² pro Stunde · ${scenarios[0]?.month || ''} · ${scenarios.length} Szenario(s)`;

  drawProfileChart(yields, area, pr);
  drawCumulativeChart(yields, area, pr);
  drawBarChart(yields);
  drawDonutChart(best);
  drawLegend();
}

function drawProfileChart(yields, area, pr) {
  APP.destroyChart('sim-prof');
  const labels = Array.from({length:24}, (_,i) => String(i).padStart(2,'0')+':00');
  const datasets = scenarios.map((s,i) => {
    const hourData = Array(24).fill(0);
    s.data.forEach(d => { hourData[d.hour] = d.gi; });
    return {
      label: s.label,
      data: hourData,
...
function drawCumulativeChart(yields, area, pr) {
  APP.destroyChart('sim-cum');
  const labels = Array.from({length:24}, (_,i) => String(i).padStart(2,'0')+':00');
  const datasets = scenarios.map((s,i) => {
    const hourData = Array(24).fill(0);
    s.data.forEach(d => { hourData[d.hour] = d.gi; });
    // Cumulative kWh: sum up to each hour * 1h interval / 1000 * area * PR
    let cum = 0;
    const cumData = hourData.map(gi => {
      cum += gi / 1000 * area * pr;
...
function drawBarChart(yields) {
  APP.destroyChart('sim-bar');
  const labels = scenarios.map(s => s.label);
  const bgColors = scenarios.map((_,i) => COLORS[i%COLORS.length] + 'cc');
  APP.charts['sim-bar'] = new Chart(document.getElementById('sim-c-bar'), {
    type: 'bar',
    data: { labels, datasets:[{ data:yields, backgroundColor:bgColors, borderRadius:4 }] },
    options: { responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}},
      scales: { x:{ticks:{color:APP.TC,font:{size:10},maxRotation:30},grid:{color:APP.GC}},
                y:{ticks:{color:APP.TC,font:{size:10}},grid:{color:APP.GC},
                   title:{display:true,text:'kWh/Tag',color:APP.TC,font:{size:10}}} } }
  });
}

function drawDonutChart(best) {
...
function removeScenario(idx) {
  ...
  const listEl = document.getElementById('sim-file-list');
  if (listEl) listEl.textContent = scenarios.length > 0 ? `✓ ${scenarios.length} Szenario(s) geladen` : '';
  render();
}

function clearAll() {
  scenarios = [];
  const fi = document.getElementById('sim-file');
  if (fi) fi.value = '';
  const listEl = document.getElementById('sim-file-list');
  if (listEl) listEl.textContent = '';
  const scenEl = document.getElementById('sim-scenarios');
  if (scenEl) scenEl.innerHTML = '';
  const metEl = document.getElementById('sim-metrics');
  if (metEl) metEl.style.display = 'none';
  ['sim-prof','sim-cum','sim-bar','sim-donut'].forEach(id => APP.destroyChart(id));
}

function register() {
  APP.registerPage('simulation', { html: HTML, onEnter: () => {} });
}

return { loadFiles, render, removeScenario, clearAll, register };
})();
